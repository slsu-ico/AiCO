...
test('schema constrains roles, statuses, and content types', () => {
  ...
  assert.match(
    schema,
    /role\s+text\s+NOT NULL[\s\S]+CHECK\s*\(\s*role\s+IN\s*\('admin',\s*'office_user'\)\s*\)/i,
  );
  assert.match(
    schema,
    /status\s+text\s+NOT NULL\s+DEFAULT 'pending'\s+CHECK\s*\(\s*status\s+IN\s*\('pending',\s*'approved',\s*'rejected',\s*'needs_info'\)\s*\)/i,
  );
  assert.match(
    schema,
    /content_type\s+text\s+NOT NULL\s+CHECK\s*\(\s*content_type\s+IN\s*\('citizens_charter_service',\s*'faq',\s*'event',\s*'project',\s*'program',\s*'activity'\)\s*\)/i,
  );
  assert.match(
    schema,
    /status\s+text\s+NOT NULL\s+DEFAULT 'draft'\s+CHECK\s*\(\s*status\s+IN\s*\('draft',\s*'pending_review',\s*'published',\s*'rejected',\s*'needs_revision',\s*'archived'\)\s*\)/i,
  );
});
...
test('schema defines account request columns for office onboarding review', () => {
  ...
  assertColumn(
    accountRequests,
    'office_id',
    'bigint\\s+REFERENCES offices\\(id\\) ON DELETE SET NULL',
  );
  ...
  assertColumn(
    accountRequests,
    'reviewed_by',
    'bigint\\s+REFERENCES users\\(id\\) ON DELETE SET NULL',
  );
...
test('schema defines content item and version columns for review workflow', () => {
  ...
  assertColumn(
    contentVersions,
    'content_item_id',
    'bigint\\s+NOT NULL\\s+REFERENCES content_items\\(id\\) ON DELETE CASCADE',
  );
  assertColumn(
    contentVersions,
    'version_number',
    'integer\\s+NOT NULL\\s+CHECK \\(version_number > 0\\)',
  );
  ...
  assertColumn(
    contentVersions,
    'structured_payload',
    "jsonb\\s+NOT NULL\\s+DEFAULT '\\{\\}'::jsonb",
  );
  assertColumn(
    contentVersions,
    'submitted_by',
  ...
  assert.match(
    schema,
    /FOREIGN KEY\s*\(current_published_version_id\)\s+REFERENCES content_versions\(id\)/i,
  );
});
...
test('schema defines review notes and generic attachment metadata', () => {
  ...
  assertColumn(
    reviewNotes,
    'content_version_id',
    'bigint\\s+NOT NULL\\s+REFERENCES content_versions\\(id\\) ON DELETE CASCADE',
  );
...
test('schema defines required lookup indexes', () => {
  ...
  assert.match(
    schema,
    /CREATE INDEX IF NOT EXISTS idx_account_requests_status\s+ON account_requests\(status\)/i,
  );
  assert.match(
    schema,
    /CREATE INDEX IF NOT EXISTS idx_users_email_active\s+ON users\(email\)\s+WHERE active = true/i,
  );
  assert.match(
    schema,
  ...
  assert.match(
    schema,
    /CREATE INDEX IF NOT EXISTS idx_content_versions_status\s+ON content_versions\(status\)/i,
  );
  assert.match(
    schema,
    /CREATE INDEX IF NOT EXISTS idx_content_versions_item_status\s+ON content_versions\(content_item_id,\s*status\)/i,
  );
  assert.match(
    schema,
    /CREATE INDEX IF NOT EXISTS idx_content_versions_published\s+ON content_versions\(published_at\s+DESC,\s*id\s+DESC\)\s+WHERE status = 'published'/i,
  );
});
